...
Open: **Bug O** — intermittent multi-second stalls under N>1 workers (suspected `SetLatch` race in pipeline leader / worker completion). Bug N's perf sweep at w∈{2,4,8} alternates between best-case linear speedup (w=8: 34 ms) and outliers at suspiciously round 1000 ms / 2000 ms / 5000 ms — strong signature of `WaitLatch` timeout retries firing because a completion signal is being lost. Was masked while only 1 worker ran (Bug N collapse meant N=1 always). Likely candidates: (1) `SetLatch` race when N workers atomic-dec `tasks_remaining` near-simultaneously and only the last setter reaches the leader latch; (2) head-of-line blocking in `TryPopForWorker` strict affinity when worker 0 is slow to attach; (3) COMBINE/FINALIZE event chain not waiting for all N partials. Investigate next.
...
## Q1-1M PERF SNAPSHOT (2026-04-30, post Bug L+M, median-of-5)

| config | median ms | vs native-serial |
|---|---:|---:|
| native-serial | 137.22 | 1.00× |
| native-parallel-default (PG default workers) | **52.01** | **2.64×** |
| pgvv-serial | 134.08 | 1.02× |
| pgvv-par-w1 | 168.75 | 0.81× |
| pgvv-par-w2 | 171.25 | 0.80× |
| pgvv-par-w4 | 174.71 | 0.79× |
| pgvv-par-w8 | 176.96 | 0.78× |

### Post-Bug-N sweep (2026-04-30, 5 trials, raw — bimodal due to Bug O stalls)

| config | best ms | worst ms | notes |
|---|---:|---:|---|
| pgvv-par-w1 (post-N) | 157 | 164 | clean, no stalls (N=1 → no race surface) |
| pgvv-par-w2 (post-N) | **84** | 1095 | best ≈ 2× w=1; outliers = Bug O |
| pgvv-par-w4 (post-N) | 1008 | 3010 | every trial stalled — Bug O dominates |
| pgvv-par-w8 (post-N) | **34** | 5014 | best = **4.6× speedup** vs w=1; tail catastrophic |

**Bug N proves morsel sharding works** (w=8 best 34 ms vs w=1 158 ms); **Bug O blocks shipping** the speedup until the wakeup race is fixed. The pre-Bug-N "negative scaling" diagnosis (rows pgvv-par-w1..w8 above) was actually masking a fan-out collapse to 1 worker; perf comparison vs native-parallel must be re-run after Bug O.

**Diagnosis:** (a) pgvv-serial ≈ native-serial → zero SIMD/JIT win on the hot path; (b) pgvv parallel scales NEGATIVELY w=1→8 — per-query bgworker spawn (~8-10 ms × N on macOS, unamortizable in our per-query model) plus sink/sort serialization swamps any parallel benefit at this scale; (c) native PG hides spawn cost via cost-model gating that skips parallel for cheap queries. **Strategic next step**: pivot M-Q1-PERF to serial hot-path SIMD/JIT (where the real ~85ms gap lives vs native-parallel) before attempting to fix the parallel runtime. Parallel path requires worker pooling / cross-query reuse, not a per-query model.
