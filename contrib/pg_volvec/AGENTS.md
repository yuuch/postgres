...
Open: none in M-FRAME-MIN step 3g.2-final. Q1 emits correct results end-to-end through the MetaPipeline runtime (parallel-on AND parallel-off paths) at both small fixture and 1M-row scale across workers ∈ {1,2,4}. All `PGVOLVEC_DIAG` fprintf instrumentation has been stripped from the runtime sources; AGENTS.md doc references the tag for historical context only.
