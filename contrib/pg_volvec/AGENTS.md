...
## SF=10 CANONICAL Q1 PERF (2026-04-30, post Bug Q, median-of-5)
...
|---|---|---:|
| pgvv w=8 | 8652 / 8109 / 8752 / 8573 / 8459 | **8573** |
| native w=8 | 24702 / 27056 / 70328 / 27956 / 71099 | 27956 |

**pgvv 3.26× faster than native parallel-w8**, with tight spread (σ≈250 ms) vs native bimodal (24–71 s — likely partitioned-hash spill / repartition variance under heavy `numeric_mul`/`numeric_div` per row). Correctness: `sum_*` and `count_order` byte-exact vs native; `avg_*` displays scale-2 (by-design int64-scaled numeric, see SESSION SIMPLIFICATIONS — not a regression). The relative gap vs SF=10-simple-Q1 (where native won 1.37×) is dominated by canonical Q1's expression-heavy aggregate list: pgvv's i64 scaled fast-path + vectorized expression eval scales while native's per-row varlena `numeric_*` calls do not.
