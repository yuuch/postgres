...
dsa_pointer DsaDataChunkBridge::append_data_chunk(const DataChunk<DEFAULT_CHUNK_SIZE> &chunk, bool &spilled_to_disk, BufFile *spill_file)
{
	...
	if (fallback_triggered_ || total_allocated_ + req_size > limit_)
	{
		...
		elog(LOG, "pg_volvec: DSA bridge FALLBACK to BufFile (row_count=%u, req_size=%zu, total_allocated=%zu, limit=%zu)",
	...
	if ((log_counter++ % 1000) == 0)
		elog(LOG, "pg_volvec: DSA bridge using DSA memory (row_count=%u, req_size=%zu, total_allocated=%zu, limit=%zu, chunks=%zu)",
