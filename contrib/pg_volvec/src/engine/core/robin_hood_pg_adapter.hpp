...
private:
	...
	void rehash(size_t new_cap) {
		...
		if (new_cap != 0 && alloc_size / new_cap != sizeof(Slot))
			elog(ERROR, "pg_volvec robin hood hash table allocation overflow (new_cap=%zu slot_size=%zu)",
				 new_cap, sizeof(Slot));
		if (alloc_size > ((size_t) 512 * 1024 * 1024))
			elog(ERROR,
				 "pg_volvec refusing huge robin hood hash table allocation (new_cap=%zu slot_size=%zu bytes=%zu old_cap=%zu size=%zu)",
				 new_cap,
				 sizeof(Slot),
				 alloc_size,
				 old_cap,
