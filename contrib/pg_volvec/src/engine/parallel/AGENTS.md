...
| Bridge entry into the runtime | `bridge/execute.cpp` will call `pipeline::PgvolvecPipelineRun(...)` (currently a stub returning `false`) |
...
- **Do NOT add design docs at this level.** The current authoritative plans live in `.sisyphus/plans/` (`pipeline-port-plan.md`, `3g2-final-delta-map.md`) and the design doc `contrib/pg_volvec/docs/GLOBAL_LOCAL_STATE_DESIGN.md`.
