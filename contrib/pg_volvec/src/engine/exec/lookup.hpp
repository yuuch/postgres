...
public:
	...
	bool configure_source_block_range(BlockNumber start_block, uint32_t nblocks) override
	{
		...
		if (pg_volvec_trace_hooks && !ok)
			elog(LOG,
				 "pg_volvec: lookup project block range configure failed start=%u nblocks=%u left=%s",
				 start_block,
				 nblocks,
	...
	VecAggState *find_parallel_aggregate_state_by_plan_node_id(int target_plan_node_id) override
	{
		return left_ != nullptr ?
			left_->find_parallel_aggregate_state_by_plan_node_id(target_plan_node_id) :
			nullptr;
	}
...
public:
	...
	bool configure_source_block_range(BlockNumber start_block, uint32_t nblocks) override
	{
		...
		if (pg_volvec_trace_hooks && !ok)
			elog(LOG,
				 "pg_volvec: lookup project multi block range configure failed start=%u nblocks=%u left=%s",
				 start_block,
				 nblocks,
				 left_ != nullptr ? "ok" : "null");
		return ok;
...
public:
	...
	bool configure_source_block_range(BlockNumber start_block, uint32_t nblocks) override
	{
		...
		if (pg_volvec_trace_hooks && !ok)
			elog(LOG,
				 "pg_volvec: lookup filter block range configure failed start=%u nblocks=%u left=%s",
				 start_block,
				 nblocks,
				 left_ != nullptr ? "ok" : "null");
		return ok;
