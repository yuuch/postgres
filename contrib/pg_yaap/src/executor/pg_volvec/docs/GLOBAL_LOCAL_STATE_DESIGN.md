...
### 6.3 Sink cross-process payload ABI (DSA-based — superseded shm_toc form)

> **AUTHORITATIVE.** The earlier `(shm_toc*, key)` ABI is deleted. Under the
> ownership split locked in §8.5.1 the DSM segment carries only 3 fixed keys
> (CONTROL / DSA / TASK_QUEUE); all per-sink cross-process payloads live
> inside the DSA arena and are addressed by `dsa_pointer`. This subsection
> replaces the prior `SerializeToDSM` / `DeserializeFromDSM` contract.

Each `GlobalSinkState` subclass that needs cross-worker shared payload
(`AggGlobalSinkState`, `SortGlobalSinkState`) declares two leader-only
factories that publish that payload into DSA. Workers never call these;
they consume the resulting `dsa_pointer` via the per-process Sink view (see
§8.5.3 ExecCtx-branched factories and §8.5.4 OpDescriptor reconstruction).

```cpp
// leader-only, called once during Sink::GetGlobalSinkState(ExecCtx&) when
// ExecCtx.worker_index == LEADER_WORKER_INDEX
//
//   payload_dp = dsa_allocate(ctx.dsa, sizeof(AggSharedPayload) +
//                             num_slots * sizeof(ParallelAggPartialState));
//   AggSharedPayload *p = (AggSharedPayload *) dsa_get_address(ctx.dsa, payload_dp);
//   p->num_slots = num_slots;
//   memset(p->slots, 0, num_slots * sizeof(ParallelAggPartialState));
//   // store payload_dp into OpDescriptor.global_sink_state (§8.5.4); workers
//   // read it from the DSA-resident OpDescriptor at attach time.
```

`GlobalSinkState` itself is **not** DSA-resident (it carries vtable +
process-local handles such as `dsa_area*`). What lives in DSA is the *raw
payload struct* (`AggSharedPayload`, `SortSharedPayload`, …) that every
worker must observe. The leader's `GlobalSinkState` instance owns the
allocation; the worker's `GlobalSinkState` view is a wrapper that holds
`{dsa_area*, dsa_pointer}` and resolves the payload on each access via
`dsa_get_address`. Combine/Finalize semantics are unchanged from the prior
ABI — only the addressing changes from `shm_toc` keys to DSA pointers.

This matches the DuckDB model (HashAggregate global sink state is
leader-side-only, partition tables are the cross-process payload), and
satisfies the pipeline/AGENTS.md anti-pattern *"Do NOT publish palloc'd
pointers via PipelineSharedControl or DSA"* — only DSA-allocated POD
payload structs are published, never palloc'd handles.
...
#### 8.5.3 PhysicalOperator base contract (ground-truth API)

> **AUTHORITATIVE.** This subsection is the canonical contract for the
> `PhysicalOperator` hierarchy under M-FRAME-MIN. The base headers
> (`physical_operator.hpp` + `pipeline/{source,sink,operator}.hpp`) are the
> single source of truth; they MUST NOT be modified to fit downstream code.
> Where this subsection and §8.5.2 disagree (notably the count of op tags),
> §8.5.3 wins.

##### 8.5.3.1 Concrete operator inventory

There are **exactly four** `PhysicalOperatorType` tags. There is no
standalone `Filter` op — predicate evaluation is fused into `PhysicalSeqScan`
via its `qual_` `ExprProgram`. The §8.5.2 reference to "5th `OutputSink`
tag" is stale and is overridden by this table:

| Tag | Subclass | Role(s) | Pipeline-breaker | MaxThreads | Holds |
|---|---|---|---|---|---|
| `SEQ_SCAN` | `PhysicalSeqScan` | Source only | no | shared-relation parallel (N) | `VecSeqScanState*`, `ExprProgram*` (qual), `PhysicalSeqScanShared{atomic next_block, total_blocks, morsel_nblocks}` |
| `HASH_AGGREGATE` | `PhysicalHashAggregate` | Sink + Source | yes | N | `VecAggState*` |
| `ORDER` | `PhysicalOrder` | Sink + Source | yes | 1 (single-thread merge first cut) | (no leader-side handles in M-FRAME-MIN) |
| `OUTPUT` | `OutputSink` | Sink only (terminator) | no | 1 | (no leader-side handles) |

##### 8.5.3.2 Base virtual signatures (do not modify)

```cpp
// physical_operator.hpp (shape extracted from current source — DO NOT EDIT)
class PhysicalOperator : public PgMemoryContextObject {
public:
    explicit PhysicalOperator(PhysicalOperatorType type);
    virtual ~PhysicalOperator() = default;

    PhysicalOperatorType type() const;
    void                 AddChild(std::unique_ptr<PhysicalOperator> child);
    const PgVector<std::unique_ptr<PhysicalOperator>> &children() const;
...
`OperatorSourceInput` / `OperatorSinkInput` / `OperatorSinkCombineInput` are
plain reference-bundle structs over the corresponding `Global*State` and
`Local*State`. `BLOCKED` enum values are reserved by the type system but are
**forbidden** in M-FRAME-MIN per `pipeline/AGENTS.md` anti-pattern.

##### 8.5.3.3 State lifecycle (per pipeline, per worker)

```
attach phase (worker process startup, once per query)
    deserialize OpDescriptor[]  →  reconstruct PhysicalOperator* tree (§8.5.4)

per-pipeline first-task (lazy, in Task::Execute)
    if (gstate is null for this pipeline-op)
        gstate = op.GetGlobalSourceState(ctx)   // or GetGlobalSinkState
    lstate = op.GetLocalSourceState(ctx, *gstate)   // or GetLocalSinkState

per-chunk
    op.GetData(ctx, chunk, {*gstate, *lstate})       // source
    op.SinkChunk(ctx, chunk, {*gstate, *lstate})    // sink

end of run event
    op.Combine(ctx, {*lstate, *gstate})   // worker-local → cross-worker payload

end of finalize event (LEADER ONLY)
    op.Finalize(ctx, *gstate)
```

`ExecCtx.worker_index == LEADER_WORKER_INDEX` (-1) on the leader;
non-negative on workers. Every `Get{Global,Local}*State` factory MUST branch
on `ctx.worker_index` to decide whether to allocate cross-worker DSA payload
(leader path) or to wrap the existing payload as a process-local view
(worker path) — see §8.5.4.4.

##### 8.5.3.4 ERROR-safety and MemoryContext rules

Per §15.3.2 L1957 and `pipeline/AGENTS.md` anti-patterns:

- All `Get*State` factories MUST be invoked under `MemoryContextSwitchTo(per-query-mcxt)`.
- The returned `unique_ptr<…State>` is converted into a
  `MemoryContextAlloc + placement-new + memory-context reset callback dtor`
  ownership before any `PG_TRY` block (zero stack-RAII inside `PG_TRY`).
  The reusable guard is `core::PgMcxtCallbackGuard` (B2 commit `dd4a041d343`).
- Worker-side factory calls MUST happen *after* DSM/DSA attach completes;
  before that, `ctx.dsa` is invalid.

#### 8.5.4 OpDescriptor ↔ PhysicalOperator mapping protocol

> **AUTHORITATIVE.** This subsection answers the four open questions left
> after §8.5.2: how leader serializes leader-only PG handles
> (`VecSeqScanState*`, `VecAggState*`, `ExprProgram*`,
> `PhysicalSeqScanShared`) into DSA POD form, and how workers reconstruct
> equivalent operator instances without ever touching `PlannedStmt`.

##### 8.5.4.1 Design constraints (recap)

1. **No palloc'd pointers in DSA.** OpDescriptor must be POD; only
   `dsa_pointer`, primitives, and inline POD structs.
2. **No `PlannedStmt` in worker.** The user has explicitly forbidden
   PlannedStmt / query-text DSM publication. Workers reconstruct ops purely
   from DSA-resident `OpDescriptor[]`.
3. **No base-header changes.** The factory signatures in §8.5.3.2 are
   frozen; `Get*State(ExecCtx&)` must remain the only entry point.
4. **`Global*State` is conceptually a single shared payload** owned by the
   leader; per-process instances are *views* over the DSA-resident payload.

##### 8.5.4.2 OpDescriptor POD layout (tagged union)

```cpp
// pipeline/pipeline_descriptor.hpp (NEW in 3g.2)

enum class OpKind : uint8_t { SEQ_SCAN, HASH_AGGREGATE, ORDER, OUTPUT };

// Schema descriptor (column types, widths) — POD, inlined.
struct ColumnSchema {
    Oid       type_oid;
    int16_t   typlen;        // negated for varlena
    bool      typbyval;
};
...
// Bytecode-form expression program (Q2: ExprProgram → POD bytecode).
// Pure POD: opcode stream + operand index stream + constant pool bytes.
// Worker reconstructs an interpreter ExprProgram by feeding these buffers
// to a restricted ExprProgram::DeserializeBytecode(ctx.mcxt, bytes) factory.
struct ExprBytecode {
    uint32_t   n_insns;
    uint32_t   n_consts;
    uint32_t   const_pool_bytes;
    // Followed inline by:
    //   Insn         insns[n_insns];          // opcode + 3 operand indices
    //   ConstSlot    consts[n_consts];        // {type_oid, offset_in_pool, len}
    //   uint8_t      const_pool[const_pool_bytes];
};

// Per-op tagged-union body. All variants are POD.
struct SeqScanOpBody {
    Oid              relid;                 // worker re-opens via relation_open
    SchemaDescriptor input_schema;          // worker rebuilds VecSeqScanState
    SchemaDescriptor output_schema;         // post-projection (= input projection)
    dsa_pointer      qual_bytecode;         // → ExprBytecode in DSA, or InvalidDsaPointer
    // PhysicalSeqScanShared cross-worker atomic + sizing (DSA-resident copy):
    dsa_pointer      shared_payload;        // → SeqScanSharedPayload in DSA
};
...
struct HashAggOpBody {
    ...
    // Followed by inline arrays (allocated as part of the same dsa_allocate):
    //   uint16_t       group_key_indices[n_group_keys];
    //   AggFuncDesc    agg_funcs[n_agg_funcs];   // {oid, input_col_idx, transtype, finaltype}
    dsa_pointer      shared_payload;        // → AggSharedPayload (§6.3)
};
...
struct OrderOpBody {
    ...
    // Followed by inline:
    //   SortKeyDesc  sort_keys[n_sort_keys];     // {col_idx, asc, nulls_first, collation_oid}
    dsa_pointer      shared_payload;        // → SortSharedPayload, MaxThreads=1
};

struct OutputOpBody {
    SchemaDescriptor input_schema;
    // (no shared payload — sink terminator)
};
...
struct OpDescriptor {
    ...
    uint8_t    n_children;
    uint16_t   _pad;
    uint32_t   child_indices[4];   // indices into Pipeline.ops_dsa[] array
    union {
...
All inline arrays are flexible-array members or trailing tails of the same
`dsa_allocate` block; cross-block references go through `dsa_pointer`.
Total per-op size is bounded (no recursive embedding) so each OpDescriptor
is a single `dsa_allocate` call.

##### 8.5.4.3 Leader serialization algorithm

```
LeaderSerializePipelines(MetaPipelineBundle &bundle, dsa_area *dsa,
                         dsa_pointer *out_root)
  pipelines_dp = dsa_allocate(dsa, sizeof(PipelineDescriptor) *
                              bundle.pipelines.size())
  for each Pipeline p in bundle.pipelines:
      pd = &pipelines_dp[p.id]
      pd.pipeline_id      = p.id
      pd.dependency_mask  = bitmask(p.depends_on)
      pd.op_count         = 1 + p.ops.size() + 1   // source + ops + sink
      pd.ops              = SerializeOpsArray(p, dsa)
      pd.global_*_state   = InvalidDsaPointer      // populated lazily
      pg_atomic_init_u32(&pd.task_slot_next, 0)
  *out_root = pipelines_dp

SerializeOpsArray(Pipeline &p, dsa_area *dsa) → dsa_pointer
  ops_dp = dsa_allocate(dsa, sizeof(OpDescriptor) * pd.op_count)
  for each PhysicalOperator *op in concat(source, ops, sink):
      switch op->type():
        case SEQ_SCAN:
            EmitSeqScan(static_cast<PhysicalSeqScan*>(op), dsa, &ops_dp[i])
        case HASH_AGGREGATE:
            EmitHashAgg(static_cast<PhysicalHashAggregate*>(op), dsa, &ops_dp[i])
        case ORDER: ... ;  case OUTPUT: ...
  return ops_dp

EmitSeqScan(PhysicalSeqScan *op, dsa_area *dsa, OpDescriptor *out)
  out->kind = SEQ_SCAN
  // VecSeqScanState* is NOT serialized; leader extracts only POD metadata:
  out->body.seq_scan.relid          = op->vec_state_->relid
  out->body.seq_scan.input_schema   = SerializeSchema(op->vec_state_->tupdesc)
  out->body.seq_scan.output_schema  = SerializeSchema(op->vec_state_->out_tupdesc)
  out->body.seq_scan.qual_bytecode  = op->qual_
        ? SerializeExprProgram(op->qual_, dsa)
        : InvalidDsaPointer
  // PhysicalSeqScanShared.next_block is already a pg_atomic_uint64 — but it
  // currently lives in leader-process memory. Re-allocate inside DSA:
  shared_dp = dsa_allocate(dsa, sizeof(SeqScanSharedPayload))
  shared = (SeqScanSharedPayload*) dsa_get_address(dsa, shared_dp)
  pg_atomic_init_u64(&shared->next_block, 0)
  shared->total_blocks  = op->shared_.total_blocks
  shared->morsel_nblocks = op->shared_.morsel_nblocks
  out->body.seq_scan.shared_payload = shared_dp
```

`SerializeExprProgram` walks the existing linear IR from `expr.cpp`, emits
opcode/operand/const arrays into a single `dsa_allocate`'d `ExprBytecode`
block, and returns the `dsa_pointer`. ExprProgram has no pointer-typed
operands; constants are copied by value (datums + length for varlenas).

##### 8.5.4.4 Worker reconstruction algorithm

Worker enters via `pg_volvec_pipeline_worker_main` (§8.5.2 worker
entrypoint). After DSM/DSA attach:

```
WorkerReconstructPipelines(PipelineSharedControl *ctl, ExecCtx &worker_ctx)
  pipelines_dp = ctl->pipelines_root
  num_pipelines = ctl->num_pipelines
  // Walk DSA-resident PipelineDescriptor[] and rebuild Pipeline + ops in
  // per-query MemoryContext. PhysicalOperator instances are FRESHLY
  // constructed; they are NOT copies of leader heap objects.
  for each i in [0, num_pipelines):
      pd = (PipelineDescriptor*) dsa_get_address(dsa, pipelines_dp[i])
      reconstructed[i] = ReconstructPipeline(pd, worker_ctx)

ReconstructPipeline(PipelineDescriptor *pd, ExecCtx &ctx) → Pipeline*
  pipeline = MemoryContextAllocZero(ctx.mcxt, sizeof(Pipeline))
  for each j in [0, pd->op_count):
      ops_dp_j   = ((OpDescriptor*) dsa_get_address(dsa, pd->ops)) + j
      op = ReconstructOp(ops_dp_j, ctx)
      AttachOpToPipeline(pipeline, op, j, pd->op_count)
  return pipeline

ReconstructOp(OpDescriptor *od, ExecCtx &ctx) → PhysicalOperator*
  switch od->kind:
    case SEQ_SCAN:
        // Worker freshly opens the relation and builds VecSeqScanState in
        // worker mcxt. NO leader pointer is ever followed.
        rel = relation_open(od->body.seq_scan.relid, AccessShareLock)
        vss = WorkerBuildVecSeqScanState(ctx.mcxt, rel,
                                         od->body.seq_scan.input_schema,
                                         od->body.seq_scan.output_schema)
        qual = od->body.seq_scan.qual_bytecode == InvalidDsaPointer
               ? nullptr
               : ExprProgram::DeserializeBytecode(
                     ctx.mcxt,
                     (ExprBytecode*) dsa_get_address(dsa,
                         od->body.seq_scan.qual_bytecode))
        // Worker-side PhysicalSeqScanShared wraps the DSA-resident payload
        // by pointer (the atomic lives in DSA, so workers and leader share it):
        shared = (SeqScanSharedPayload*) dsa_get_address(dsa,
                     od->body.seq_scan.shared_payload)
        return new (MemoryContextAlloc(ctx.mcxt, sizeof(PhysicalSeqScan)))
                   PhysicalSeqScan(vss, qual, shared);

    case HASH_AGGREGATE:
        vas = WorkerBuildVecAggState(ctx.mcxt,
                                     od->body.hash_agg.input_schema,
                                     od->body.hash_agg.output_schema,
                                     od->body.hash_agg.n_group_keys,
                                     od->body.hash_agg.group_key_indices(),
                                     od->body.hash_agg.n_agg_funcs,
                                     od->body.hash_agg.agg_funcs())
        return new (...) PhysicalHashAggregate(vas, od->body.hash_agg.shared_payload);

    case ORDER:  return new (...) PhysicalOrder(od->body.order.shared_payload);
    case OUTPUT: return new (...) OutputSink();
```

`WorkerBuildVecSeqScanState` and `WorkerBuildVecAggState` are new
worker-side constructors (3g.2 deliverable) that take *only POD descriptors*
— they are forbidden from accessing any leader heap pointer. Their bodies
do exactly what the existing leader builders do, except they receive
column metadata via `SchemaDescriptor` instead of via `TupleDesc`/`Plan*`.

##### 8.5.4.5 Global*State per-process view (Q3 resolution)

The base factory `unique_ptr<GlobalSinkState> GetGlobalSinkState(ExecCtx&)`
remains the single entry point. Implementations branch on
`ctx.worker_index`:

```cpp
std::unique_ptr<GlobalSinkState>
PhysicalHashAggregate::GetGlobalSinkState(ExecCtx &ctx) override {
    auto state = std::make_unique<AggGlobalSinkState>(ctx);
    if (ctx.worker_index == LEADER_WORKER_INDEX) {
        // Leader path: allocate AggSharedPayload in DSA (§6.3) and stamp
        // the dsa_pointer into the OpDescriptor for workers to discover.
        state->shared_payload_dp = AllocateAggSharedPayloadInDSA(ctx.dsa, ...);
        StoreSharedPayloadOnDescriptor(this, state->shared_payload_dp);
    } else {
        // Worker path: read the dsa_pointer from the OpDescriptor and
        // wrap as a per-process view. The view's dtor does NOT free the
        // payload — only the leader frees on Finalize.
...
`AggGlobalSinkState` thus has the same C++ type on leader and worker, but
the worker instance is a wrapper holding `{dsa_area*, dsa_pointer}`; all
shared data access goes through `dsa_get_address(dsa, shared_payload_dp)`.
This satisfies the base factory contract without modifying its signature
and without `reinterpret_cast` over a vtable.

`Combine(ctx, {lstate, gstate})` runs on every worker post-Run; the
implementation merges `lstate->local_partial` into the DSA-resident
`AggSharedPayload` under the appropriate atomic / lock-free pattern.
`Finalize(ctx, gstate)` runs leader-only and produces the final result
chunk(s) handed to the downstream `OutputSink`.

`PhysicalSeqScan::GetGlobalSourceState` and `PhysicalOrder::Get*State`
follow the same per-process-view template; `OutputSink::GetGlobalSinkState`
returns a no-op view (no cross-worker payload — terminator delivers chunks
to leader only).

##### 8.5.4.6 What lives where (summary)

| Artifact | Lives in | Lifetime | Owner |
|---|---|---|---|
| `PhysicalOperator` tree (leader) | leader per-query mcxt | per-query | `MetaPipelineBundle` unique_ptr |
| `PhysicalOperator` tree (worker) | worker per-query mcxt | per-query | freshly placement-new'd from `OpDescriptor[]` |
| `PipelineDescriptor[]` | DSA | per-query | leader allocates, all attach |
| `OpDescriptor[]` per pipeline | DSA | per-query | leader allocates, all attach |
| `ExprBytecode` per qual / agg | DSA | per-query | leader allocates, all attach |
| `SeqScanSharedPayload.next_block` (atomic) | DSA | per-query | leader allocates, all increment |
| `AggSharedPayload` / `SortSharedPayload` | DSA | per-query | leader allocates (§6.3) |
| `VecSeqScanState` / `VecAggState` (per process) | per-query mcxt (each process) | per-query | freshly built from `SchemaDescriptor` |
| `Global*State` C++ object (per process) | per-query mcxt (each process) | per-query | leader builds full; worker builds wrapper view |
| `Local*State` C++ object | per-query mcxt | per-task | factory in `Task::Execute` |
| `PG PlannedStmt` / `Plan` nodes | leader only | leader query lifetime | PG executor |

##### 8.5.4.7 ExprProgram POD-bytecode constraint

For Q2 to hold, `ExprProgram` IR must be designed (or refactored) so that:

- All operands are integer indices (input column index, constant pool
  offset, scratch register index) — no raw pointers.
- Constants are stored as `{type_oid, byval datum | varlena bytes}` —
  varlenas are copied by value into the const pool, not referenced.
- Function dispatch uses `Oid` / opcode enum, never `FmgrInfo*` — workers
  rebuild `FmgrInfo` from `Oid` via `fmgr_info_cxt(mcxt, ...)`.
- Bytecode is endian-defined as host-byte-order (no cross-arch concern: PG
  parallel workers share the same backend binary).

Any ExprProgram feature that would require a leader-process pointer (e.g.
Plan-node back-references, leader-allocated per-aggregate context) is
forbidden in qual / agg-projection bytecode under M-FRAME-MIN.

##### 8.5.4.8 What 3g.2 delivers (acceptance pointers)

- `pipeline/pipeline_descriptor.{hpp,cpp}` NEW — POD layout above + leader
  serialize functions + worker reconstruct functions.
- `pipeline/physical_seq_scan.cpp` EDIT — adds `GetGlobalSourceState` /
  `GetLocalSourceState` / `GetData` bodies with the ExecCtx-branched view
  pattern.
- `pipeline/physical_hash_aggregate.cpp` EDIT — full Sink/Source bodies +
  Combine/Finalize against `AggSharedPayload`.
- `pipeline/physical_order.cpp` EDIT — Sink/Source bodies (MaxThreads=1
  short-circuit OK).
- `pipeline/output_sink.cpp` EDIT — minimal Sink body.
- `pipeline/dsm_control.hpp` SHRINK — 9 keys → 3
  (`CONTROL`, `DSA`, `TASK_QUEUE`); `PipelineSharedControl` to
  `{magic, num_pipelines, dsa_pointer pipelines_root, pg_atomic_uint32 worker_error}`.
- `pipeline/pipeline_leader.cpp` REWRITE — drives Translate → MetaPipeline
  → `LeaderSerializePipelines` → DSM publish → `LaunchParallelWorkers`.
- `pipeline/pipeline_worker_main.cpp` REWRITE — attach → reconstruct →
  task pump per §8.5.2 entrypoint.
- `expr/expr.hpp` / `expr.cpp` EDIT — add `ExprProgram::SerializeBytecode`
  / `DeserializeBytecode` per §8.5.4.7 if not already POD.
