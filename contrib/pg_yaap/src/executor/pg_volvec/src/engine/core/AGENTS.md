...
| Type system | `types.hpp` | `VecType`, NUMERIC(15,2) as scaled int64, Wide128 handling. Closes its own `namespace pg_volvec` (no namespace leak) |
...
- **Namespace**: every header in this directory opens `namespace pg_volvec { … }`
...
- **Do NOT re-open the namespace leak via `types.hpp`.** Every header that
  ...
  `parallel_dsa_bridge.{hpp,cpp}`) must open its own `namespace pg_volvec`
  block.
