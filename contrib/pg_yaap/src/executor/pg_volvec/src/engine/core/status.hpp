...
/*
 * core/status.hpp — return-code error model for pg_volvec C++ kernel (B2.1).
 *
 * pg_volvec is built with -fno-exceptions. The C++ kernel under
 * parallel/pipeline/ propagates failures by value via Status / expected<T>.
 * The ONLY PG_TRY/PG_CATCH boundary lives at the C/C++ edge in
 * bridge/execute.cpp; PG C calls that may ereport() must be wrapped at
 ...
 *   }
 */
#define PG_VOLVEC_TRY(expr)                                       \
	do {                                                          \
		::pg_volvec::Status _pgv_st = (expr);                     \
