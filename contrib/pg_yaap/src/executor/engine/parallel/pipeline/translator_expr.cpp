...
{
	(void) op;
	const bool is_eq = std::strcmp(opname, "=") == 0;
	const bool is_ne = std::strcmp(opname, "<>") == 0 || std::strcmp(opname, "!=") == 0;
	if (!is_eq && !is_ne)
		return false;
	ColumnRef ref{};
	...
	if (next_int64_slot >= 16)
		return false;
	out_result_slot = next_int64_slot++;
	steps.push_back(ProjectStep{is_eq ? ProjectOp::STRING_EQ_VAR_CONST : ProjectOp::STRING_NE_VAR_CONST,
		col->chunk_slot,
		const_len,
		out_result_slot,
		const_value});
				     ...
				     uint8_t &out_slot)
{
	...
	if (nodeTag(expr) == T_OpExpr)
	{
		...
		if (opname == nullptr)
			return false;
		const bool ok = LowerStringPrefixLike(op,
			steps,
			next_int64_slot,
			raw_cols_ref,
			raw_cols,
			context_plan,
			out_slot) || LowerStringCompareExpr(op,
		       ...
		       int16_t &out_numeric_scale)
{
	...
	if (IsBareVarArg(arg, context_plan, bare_ref))
	{
		...
		if (!ColumnNumericScale(*col, scale))
			return false;
		out_desc.input_col_idx = col->chunk_slot;
		out_numeric_scale = (out_kind == TdcAggKind::AVG_NUMERIC) ?
			static_cast<int16_t>(scale + kAvgNumericExtraScale) : scale;
		return true;
	}
	int8_t lowered_scale = 0;
	uint8_t lowered_slot = 0;
	...
	out_desc.input_col_idx = lowered_slot;
	out_numeric_scale = (out_kind == TdcAggKind::AVG_NUMERIC) ?
		static_cast<int16_t>(lowered_scale + kAvgNumericExtraScale) : lowered_scale;
	return true;
}
