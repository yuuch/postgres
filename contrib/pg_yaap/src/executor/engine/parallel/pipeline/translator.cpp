...
static bool
TryExtractLimitCount(Limit *limit, uint64_t &out_count)
{
	out_count = 0;
	if (limit == nullptr)
		return false;
	if (limit->limitOffset != nullptr)
		return false;
	if (limit->limitCount == nullptr)
		return false;

	Expr *expr = (Expr *) limit->limitCount;
	if (expr == nullptr || nodeTag(expr) != T_Const)
		return false;
	...
	{
		case INT8OID:
			out_count = static_cast<uint64_t>(DatumGetInt64(c->constvalue));
			return true;
		case INT4OID:
			out_count = static_cast<uint64_t>(DatumGetInt32(c->constvalue));
			return true;
		case INT2OID:
			out_count = static_cast<uint64_t>(DatumGetInt16(c->constvalue));
			return true;
		default:
			return false;
	}
		    ...
		    DerivedAggBuildState &out)
{
	...
	for (Aggref *aggref : aggrefs)
	{
		...
		{
			elog(LOG,
				 "pg_yaap: ClassifyAggref failed agg_idx=%zu agg_oid=%u arg_tag=%d",
				 agg_idx,
				 aggref->aggfnoid,
			 ...
			 TupleDataLayout &out_layout)
{
	...
	uint8_t next_int64_slot = 0;
	uint8_t next_double_slot = 0;
	InitializeChunkSlotWatermarks(agg_schema, next_int32_slot, next_int64_slot, next_double_slot);
...
static bool
BuildFinalOutputSortKeys(Sort *sort,
			 const std::vector<ColumnSchema> &final_output_schema,
			 std::vector<SortKeyDesc> &out)
{
	out.clear();
	if (sort == nullptr)
	...
	for (int i = 0; i < sort->numCols; ++i)
	{
		...
		if (resno <= 0 || resno > final_output_schema.size())
			return false;
		SortKeyDesc k{};
		k.col_idx = static_cast<uint16_t>(resno - 1);
		k.collation_oid = sort->collations[i];
		k.nulls_first = sort->nullsFirst[i];
		k.asc = true;
		k._pad = 0;
		(void) MapSortOpToAscending(sort->sortOperators[i], k.asc);
		out.push_back(k);
	}
			  ...
			  std::unique_ptr<PhysicalOperator> &out)
{
	if (state == nullptr || state->runtime_dsa == nullptr || qd == nullptr ||
	    node.op == nullptr)
		return false;
	if (!node.has_output_contract)
	{
		if (node.raw_schema.empty())
			return false;
		node.final_output_schema = node.raw_schema;
		if (!BuildColumnOnlyLayout(node.final_output_schema, node.final_output_layout))
			return false;
		node.has_output_contract = true;
	}
	      ...
	      NodeTranslation &out)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_Agg:
		{
			...
			if (input_sort != nullptr &&
				out.final_sort_keys.empty() &&
				!BuildFinalOutputSortKeys(input_sort, out.final_output_schema, out.final_sort_keys))
				return false;
		...
		case T_Sort:
		{
			...
			out = std::move(child);
			if (!BuildFinalOutputSortKeys(sort, out.final_output_schema, out.final_sort_keys))
				return false;
			out.sort = sort;
			return true;
		}
		...
		case T_Limit:
		{
			...
			if (limit->plan.lefttree == nullptr || limit->plan.righttree != nullptr)
				return false;

			NodeTranslation child;
			if (!TranslateNode(limit->plan.lefttree, qd, state, child) || child.op == nullptr)
				return false;

			uint64_t limit_count = 0;
			if (!TryExtractLimitCount(limit, limit_count))
				return false;
...
Translator::TranslatePlan(Plan *plan, QueryDesc *qd, PgYaapQueryState *state)
{
	if (plan == nullptr || qd == nullptr || state == nullptr || state->runtime_dsa == nullptr)
		return nullptr;
...
std::unique_ptr<PhysicalOperator>
Translator::Translate(QueryDesc *qd, PgYaapQueryState *state)
{
	if (qd == nullptr || qd->plannedstmt == nullptr || qd->plannedstmt->planTree == nullptr)
		return nullptr;
